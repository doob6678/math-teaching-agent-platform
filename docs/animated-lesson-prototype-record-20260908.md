...
**布局重叠修复（09-09 下午，老板第二张实拍）**：结论框与公式卡叠罗汉，两个根因：
①`_answer` 用固定坐标 [4.0,1.6] 落位，正压在笔记本列的公式流上——改为进流
（`_note_slot`/`_note_commit` 与推导行同机制，宽超列先收窄）；②`_note_slot` 的
老 bug：触发滚动时新行已在旧位 move_to，滚动只移已有行，新行留在下界外
（旧片行距越挤越低即此因）——滚动后把新行重放到腾出的行位。
按老板"题干要在讲解画面里一直在"新增 `_problem_card`：题干常驻钉笔记本列顶，
$...$ 混排+贪心换行（CJK 逐字、公式原子、标点禁行首），字号阶梯 18→12 压到
列高 45% 内；纸色不透明底+add_foreground，滚动后行顶越过卡底边的**全部**淡出
（黑板写满擦最早一笔；只擦最老一行在卡高时会漏，实拍抓到"2/5"残字）。
滚动位移踩了两轮坑：Transform(group,copy) 后再手动 shift=双倍位移；改逐行
animate.shift 后又保留手动 shift 仍双倍——擦除预测坐标与显示位置错位，
漏擦旧行从卡顶露出。终版=animate.shift 单次位移，预测即真实位置。
新增 ai_gen/check_note_column.py 体检器：卡顶上方暗像素团+画面底边出框双哨兵。
验证：椭圆课 t=5/30/44/74/76 五帧+四课结论帧体检，结论框 -3≤t≤3/2 完整落列底、
与字幕带留清晰间距、无任何重叠；四课全部按终版编译器重渲，make_demo.py 重建章节时间戳。

**直角记号修复（09-09 傍晚，老板第三张实拍）**：square 样式用三点 Polygon，manim
自动闭合第三边画成三角形——改为两条 Line 拼的开口小方角（标准直角记号）。
tangent-angle（C/A 两处）与 tangent-min（B 处）重渲抽帧目检通过；q-016 同样式
待明日重生成自动覆盖。
