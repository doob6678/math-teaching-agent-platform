...
- [ ] Task 4 部分已完成但未达到整项验收：Step 1 至 Step 4 已完成，其中本次已验证跨语言字段/越权拒绝、按 runId 重载 Java 主体、旧空幂等键恢复、MySQL 行锁与旧图版本拒绝；生产 MySQL 的 `ai_usage_event` 仍缺 `cached_prompt_tokens`，因此双 worker 幂等验收与提交步骤不得提前标记。
